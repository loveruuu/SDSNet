...
#         return results
#
#     def postprocess_seg(self, preds, h, w):
#         """Postprocesses YOLO predictions and returns output detections with proto."""
#         preds = torch.nn.functional.interpolate(preds, size=(h, w), mode='bilinear', align_corners=False)
...
class MultiPredictor(BasePredictor):
    ...
    def postprocess_seg(self, preds):
        """Postprocesses YOLO predictions and returns output detections with proto."""
        preds = torch.nn.functional.interpolate(preds, size=(1944, 2592), mode='bilinear', align_corners=False)
        preds = self.sigmoid(preds)
        _, preds = torch.max(preds, 1)
        return preds
    # def postprocess_seg(self, preds, h, w):
