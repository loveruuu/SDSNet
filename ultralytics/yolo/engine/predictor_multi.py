...
class BasePredictor:
    ...
    def stream_inference(self, source=None, model=None):
        ...
        for batch in self.dataset:
            ...
            if self.args.verbose:
                extra_time = 0  # 额外加10毫秒
                LOGGER.info(f'{s}{profilers[1].dt * 1E3 + extra_time:.1f}ms')
    ...
    def save_preds(self, vid_cap, idx, save_path):
        ...
        if self.dataset.mode == 'image':
            ...
            # Convert mask to RGB
            color_mask1 = np.stack([mask1 * 255, mask1 * 0, mask1 * 0], axis=-1)
            color_mask2 = np.stack([mask2 * 0, mask2 * 0, mask2 * 255], axis=-1)
