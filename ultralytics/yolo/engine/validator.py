    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.args.task == 'multi':
            if self.training:
                ...
                "segmentation only validation performance"
                for i, label_name in enumerate(self.data['labels_list']):
                    try:
                        results = {**stats[i], **trainer.label_loss_items_val(self.loss[i].cpu() / len(self.dataloader),
                                                                              prefix='val', task=label_name)}
                        results_list.append({k: round(float(v), 5) for k, v in results.items()})
                    except:
                        # 分割任务处理 - 提取所有指标并打印
                        seg_metrics = self.seg_result[label_name]
                        pixacc = round(seg_metrics['pixacc'].avg, 4)
                        subacc = round(seg_metrics['subacc'].avg, 4)
                        iou = round(seg_metrics['IoU'].avg, 4)
                        miou = round(seg_metrics['mIoU'].avg, 4)

                        LOGGER.info(colorstr(
                        f"{label_name}: PixAcc={pixacc:.4f}, SubAcc={subacc:.4f}, IoU={iou:.4f}, mIoU={miou:.4f}"))

                        # 构建完整的结果字典（不仅包含fitness，还包含所有分割指标）
                        dic = {
                            f'val/{label_name}/pixacc': pixacc,
                            f'val/{label_name}/subacc': subacc,
                            f'val/{label_name}/IoU': iou,
                            f'val/{label_name}/mIoU': miou,
                            'fitness': iou + miou  # 保持原有的fitness计算逻辑
                        }
                        results_list.append(dic)
