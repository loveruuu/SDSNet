...
__all__ = [
    ...
    'SpatialAttention', 'CBAM', 'Concat', 'RepConv', 'Concat_dropout', 'SelectiveGatedFusion']
...
class SelectiveGatedFusion(nn.Module):
    def __init__(self, num_inputs, reduction=16, dimension=1):
        super().__init__()
        self.d = dimension
            ...
            if i >= len(self.attentions):
                self.attentions.append(
                    nn.Sequential(
                        ...
                        nn.Flatten(),             # [B, C]
                        nn.Linear(c, max(c // self.reduction, 1)),
                        nn.SiLU(inplace=True),
                        nn.Linear(max(c // self.reduction, 1), c),
                        nn.Sigmoid(),
                        nn.Unflatten(1, (c, 1, 1)) # 回到 [B, C, 1, 1]
                    ).to(feat.device)
                )
            att = self.attentions[i](feat)      # [B, C, 1, 1]
            feat_sel = feat * att
            weights = torch.softmax(self.weight, dim=0)
            feat_gate = feat_sel * weights[i]
            outs.append(feat_gate)
