...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    Segment, Segment2, Concat_dropout, Detect_LSCSBD, SelectiveGatedFusion,
                                    CAPP, SAKA, ASPP, SPPCSPC, PSA)
from ultralytics.yolo.utils import DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, colorstr, emojis, yaml_load
from ultralytics.yolo.utils.checks import check_requirements, check_suffix, check_yaml
from ultralytics.yolo.utils.plotting import feature_visualization, feature_visualization_merged
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_LSCSBD, Segment, Segment2)):
...
#         return x
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            if isinstance(m, (Detect, Detect_LSCSBD, Segment, Segment2)):  # if it's a task head
                outputs.append(x)
            # y.append(x)
            y.append(x if m.i in self.save else None)  # save output
            if visualize:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        for m in self.model:
            if isinstance(m, (Detect, Detect_LSCSBD, Segment, Segment2, Pose)):
                s = 256  # 2x min stride
    ...
    def __init__(self, cfg='yolov8-bdd.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # m = self.model[-1]  # Detect()
            if isinstance(m, (Detect, Detect_LSCSBD, Segment, Segment2, Pose)):
                ...
                forward = lambda x: self.forward(x)[count][0] if isinstance(m, (Segment, Segment2, Pose)) else self.forward(x)[count]
                m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])  # forward
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_LSCSBD, Segment, Segment2):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF,
                 ASPP, SPPCSPC, CAPP, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, PSA, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, PSA, C3, C3TR, C3Ghost, C3x, RepC3):
        ...
        elif m is SelectiveGatedFusion:
            c2 = sum(ch[x] for x in f)
        elif m is {SAKA}:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
        elif m in (Detect, Detect_LSCSBD, Segment, Segment2, Pose, RTDETRDecoder):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect):
                return 'detect'
            elif isinstance(m, Detect_LSCSBD):
                return 'Detect_LSCSBD'
            elif isinstance(m, Segment):
                return 'segment'
            elif isinstance(m, Segment2):
                return 'segment2'
    ...
    if isinstance(model, (str, Path)):
        ...
        if '-seg' in model.stem or 'segment' in model.parts:
            return 'segment'
        elif '-seg' in model.stem or 'segment2' in model.parts:
            return 'segment2'
            ...
            return 'detect'
        elif 'Detect_LSCSBD' in model.parts:
            return 'Detect_LSCSBD'

    # Unable to determine task from model
    LOGGER.warning("WARNING ⚠️ Unable to automatically guess model task, assuming 'task=detect'. "
                   "Explicitly define task for your model, i.e. 'task=detect', 'segment', 'segment2', 'classify', or 'pose'.")
